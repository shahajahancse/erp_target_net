...
class Grid_model extends CI_Model{
		...
		foreach($query->result() as $rows)
		{
			...
			foreach($query->result() as $rows)
			{
				...
				if($status == "P")
				{
					...
					{
						$this->db->select('in_time, out_time,holiday_allowance,night_allowance');
						...
						foreach($query1->result() as $row)
						{
							...
							// $holiday_allowance = $rows->holiday_allowance;
							// $night_allowance = $rows->night_allowance;
						}
	...
	function grid_id_card($grid_emp_id)
	{
		...
		pr_emp_com_info.*,
		pr_emp_per_info.*,
		pr_emp_per_info.bangla_nam as emp_name_bn,
		pr_designation.*, 
		pr_dept.*, 
		pr_section.*,
		pr_emp_blood_groups.blood_name,
		emp_districts.name_en as emp_districts_name,
		emp_districts.name_bn as emp_districts_name_bn,
		emp_upazilas.name_en as emp_upazilas_name,
		emp_upazilas.name_bn as emp_upazilas_name_bn,
		emp_post_offices.name_en as emp_post_offices_name,
		emp_post_offices.name_bn as emp_post_offices_name_bn,
		');
		$this->db->from('pr_emp_per_info');
		$this->db->from('pr_emp_com_info');
		$this->db->from('pr_designation');
		...
		$this->db->where('pr_emp_per_info.emp_id = pr_emp_com_info.emp_id');
		$this->db->where('pr_emp_com_info.emp_desi_id = pr_designation.desig_id');
		$this->db->where('pr_emp_blood_groups.blood_id = pr_emp_per_info.emp_blood');
		$this->db->where('pr_emp_com_info.emp_dept_id = pr_dept.dept_id');
		$this->db->where('pr_emp_com_info.emp_sec_id = pr_section.sec_id');
	...
	{
		$this->db->select('pr_emp_com_info.*,pr_emp_per_info.*, pr_designation.desig_name, pr_dept.dept_name, pr_section.sec_name');
		$this->db->from('pr_emp_per_info');
		$this->db->from('pr_emp_com_info');
		$this->db->from('pr_designation');
		$this->db->from('pr_dept');
		...
		foreach($query->result() as $row)
		{
			...
			foreach($days as $day)
			{
				...
				foreach($query->result() as $row)
				{
					...
					{
						// $table = "temp_$emp_id";
						$table = 'att_'.date('Y_m',strtotime($row->shift_log_date));
	...
	function grid_actual_job_card($grid_firstdate, $grid_seconddate, $grid_emp_id)
	{
		$sStartDate = date("Y-m-d", strtotime($grid_firstdate)); 
		$sEndDate = date("Y-m-d", strtotime($grid_seconddate));
		...
		foreach($query->result() as $row)
		{
			...
			$this->db->distinct();
			$this->db->select('pr_emp_per_info.emp_full_name,pr_designation.desig_name,pr_dept.dept_name,pr_section.sec_name,pr_line_num.line_name,pr_emp_com_info.emp_join_date,pr_emp_com_info.proxi_id');
			$this->db->from('pr_emp_per_info');
			$this->db->from('pr_emp_com_info');
			$this->db->from('pr_attn_monthly');
			// $this->db->from('pr_id_proxi');
			...
			foreach($days as $day)
			{
				...
				$this->db->select('pr_emp_shift.shift_name,pr_emp_shift_log.in_time , pr_emp_shift_log.out_time, pr_emp_shift_log.shift_log_date, pr_emp_shift_log.ot_hour, pr_emp_shift_log.extra_ot_hour, pr_emp_shift_log.late_status');
				$this->db->from('pr_emp_shift_log');
				$this->db->from('pr_emp_shift');
				
				$this->db->where('pr_emp_shift_log.emp_id', $emp_id);
				$this->db->where("pr_emp_shift_log.shift_log_date", $day);
				$this->db->where('pr_emp_shift.shift_id = pr_emp_shift_log.shift_id');
				$this->db->order_by("pr_emp_shift_log.shift_log_date");
				$this->db->limit(1);
				$query = $this->db->get();
				//echo $this->db->last_query();
				foreach($query->result() as $row)
				{
					...
					$total_ot_hour = $row->ot_hour; // + 
					$total_eot_hour = $row->extra_ot_hour; 
					// , This is for extra ot hour add to Job card.
									
					$data[$emp_id]["shift_log_date"][] 	= $shift_log_date;
					$data[$emp_id]["emp_shift"][] 	= $row->shift_name;
					$data[$emp_id]["in_time"][] 		= $in_time;
					$data[$emp_id]["out_time"][] 		= $out_time;
					$data[$emp_id]["ot_hour"][] 		= $total_ot_hour;
					$data[$emp_id]["extra_ot_hour"][] 	= $total_eot_hour;
					$data[$emp_id]["att_status"][] 		= $att_status;
					$data[$emp_id]["att_status_count"][] = $att_status_count;
					$data[$emp_id]["lunch_out"][] 		= $lunch_out;
					$data[$emp_id]["lunch_in"][] 		= $lunch_in;
	...
	function salary_summary($salary_month,$emp_stat)
	{
		...
		foreach($query->result() as $rows)
		{
			...
			//For Cash 	friday_allowance
			$column_name = "friday_allowance" ;
			$friday_allowance_cash = $this->get_sum_column($column_name,$emp_id_cash,$salary_month);
			$cash_total_allowance = $friday_allowance_cash;
			$all_data["friday_allowance_cash"][] = $friday_allowance_cash;	
			
			//For Bank holiday_allowance_bank
			$friday_allowance_bank = $this->get_sum_column($column_name,$emp_id_bank,$salary_month);
			$bank_total_allowance = $friday_allowance_bank;
			$all_data["friday_allowance_bank"][] = $friday_allowance_bank;	
	...
	function factory_salary_summary_fixed($salary_month,$prev_salary_month,$emp_stat)
	{
		...
		$year_month = date('Y-m', strtotime($salary_month));
		$sal_year_month = "$year_month-01";
		$this->db->select('sec_id,sal_type,sec_sl');
		$this->db->from('pr_pay_scale_sheet');
		$this->db->where("pr_pay_scale_sheet.salary_month = '$sal_year_month'");
		$this->db->or_where("pr_pay_scale_sheet.salary_month = '$prev_salary_month'");
		//$this->db->order_by("pr_pay_scale_sheet.sec_id",'ASC');
		$this->db->order_by("pr_pay_scale_sheet.sec_sl","ASC");
		$this->db->group_by("pr_pay_scale_sheet.sec_id");
		...
		foreach($query->result() as $rows)
		{
			...
			$sec_id = $rows->sec_id;
			$all_data["sec_name"][] = $this->get_section_name($sec_id);
			$all_data["sec_type"][] = $this->get_section_type($sec_id);
			// For Cash Man Power
			$salary_draw_cash = 1;
			$emp_cash = $this->count_empid_for_salary($sec_id,$emp_stat,$salary_month,$salary_draw_cash,"count");
			$all_data["emp_cash"][] = $emp_cash;
   ...
   function get_allowance_data($emp_id,$ot_entitle,$att_date)
	...
	{
		$this->db->select("sec_name, indexing");
		$this->db->where("sec_id", $section_id);
		// if ($sec_type==2) {
		// 	$this->db->where("sec_type", $sec_type);			
	...
	function get_section_type($section_id)
	{
		$this->db->select("sec_type");
		$this->db->where("sec_id", $section_id);
		$query = $this->db->get('pr_section');
		foreach($query->result() as $rows)
		{
	...
	function get_prev_month_info($emp_id,$type)
	{
	...
	function new_join_letter_report($grid_emp_id,$type){
		// dd($grid_emp_id);
		$data = array();
		$this->db->select('
						pr_emp_com_info.emp_id,
						pr_emp_com_info.work_type,
						pr_emp_per_info.*,
						pr_designation.desig_name,
						pr_emp_com_info.emp_join_date, 
						pr_dept.dept_name, 
						...
						pre_dist.name_bn as pre_dist,
						per_dist.name_bn as per_dist,
						nomi_dist.name_bn as nomi_dist,
						pre_post.name_bn as pre_post,
						per_post.name_bn as per_post,
						nomi_post.name_bn as nomi_post,
						pre_upa.name_bn as pre_upa,
						per_upa.name_bn as per_upa,
						nomi_upa.name_bn as nomi_upa,
						pr_emp_blood_groups.blood_name
						');
		$this->db->from('pr_emp_per_info');
		$this->db->join('pr_emp_com_info', 'pr_emp_per_info.emp_id = pr_emp_com_info.emp_id');
		$this->db->join('pr_emp_blood_groups', 'pr_emp_blood_groups.blood_id = pr_emp_per_info.emp_blood');
		...
		$this->db->join('pr_emp_shift', 'pr_emp_com_info.emp_shift = pr_emp_shift.shift_id');
		$this->db->join('emp_districts as pre_dist', 'pre_dist.id = pr_emp_per_info.pre_district');
		$this->db->join('emp_districts as per_dist', 'per_dist.id = pr_emp_per_info.per_district');
		$this->db->join('emp_districts as nomi_dist', 'nomi_dist.id = pr_emp_per_info.nomi_district');
		$this->db->join('emp_post_offices as pre_post', 'pre_post.id = pr_emp_per_info.pre_post');
		$this->db->join('emp_post_offices as per_post', 'per_post.id = pr_emp_per_info.per_post');
		$this->db->join('emp_post_offices as nomi_post', 'nomi_post.id = pr_emp_per_info.nomi_post');
		$this->db->join('emp_upazilas as pre_upa', 'pre_upa.id = pr_emp_per_info.pre_upazila');
		$this->db->join('emp_upazilas as per_upa', 'per_upa.id = pr_emp_per_info.per_upazila');
		$this->db->join('emp_upazilas as nomi_upa', 'nomi_upa.id = pr_emp_per_info.nomi_upazila');
		$this->db->where_in('pr_emp_com_info.emp_id', $grid_emp_id);
		$query = $this->db->get();
		if($query->num_rows() > 0){
			return $query->result();
	...
	function leave_from($grid_firstdate, $grid_seconddate,$grid_emp_id){

		// dd($grid_firstdate.' == ' . $grid_seconddate);

		$this->db->select('pr_emp_com_info.emp_id,
			pr_emp_com_info.id,
			pr_emp_com_info.emp_position_id,
			pr_emp_per_info.emp_full_name,
			pr_emp_per_info.bangla_nam,
			emp_depertment.dept_name,
			emp_depertment.dept_name,
